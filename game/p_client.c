...
*/
//void SP_info_player_intermission(void) tg626
...
void SelectSpawnPoint(edict_t* ent, vec3_t origin, vec3_t angles)
{
    ...
    if (deathmatch->value)
        ...
        spot = SelectDeathmatchSpawnPoint(); //SelectCoopSpawnPoint (ent);
...
void ClientDisconnect(edict_t* ent)
{
    ...
    ent->s.modelindex = 0;
    ent->solid = SOLID_NOT;
    //id bug fix... otherwise the player prediction can get screwed (Not found in ID Sources? -TG626)
    ent->svflags |= SVF_NOCLIENT;
    ent->inuse = false;
